...
/* Info box (st.info) 風格覆寫，與主題配色保持一致 */
div[role="alert"] {{
...
def calculate_investment(edited_df, total_budget, fee_rate):
    ...
    for _, row in edited_df.iterrows():
        ...
        if price > 0:
            ...
            # 額外檢查：如果最低手續費導致總成本超支，則減少一股 (極少發生，但增加穩定性)
            if total_cost > allocated_budget and shares_to_buy > 0:
                shares_to_buy -= 1
                total_cost = (shares_to_buy * price) + max(MIN_FEE, round(price * shares_to_buy * fee_rate))
                estimated_fee = max(MIN_FEE, round(price * shares_to_buy * fee_rate))
        else:
            shares_to_buy = 0
            estimated_fee = 0
            total_cost = 0.0
...
edited_df = st.data_editor(
    ...
    disabled=["標的代號", "設定比例"], # <--- NEW: 鎖定標的代號與比例，只允許修改價格
    num_rows="fixed",
    key="data_editor_key",
...
st.divider()
st.caption("📌 計算邏輯：優先確保買入股數最大化，且總花費不超過分配預算；手續費最低 1 元計算。")import streamlit as st
import pandas as pd
import yfinance as yf
from datetime import datetime
